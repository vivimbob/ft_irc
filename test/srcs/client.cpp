...
Client::Client(int fifo, std::string test_case)
{
    ...
    m_ircserv_fd =
        execute(m_command + std::to_string(IRCSERV) + " > " + m_log_file +
                ".ircserv" + " < " + m_ircserv_fifo_file);
    ...
    m_ergo_fd = execute(m_command + std::to_string(ERGO) + " > " + m_log_file +
                        ".ergo" + " < " + m_ergo_fifo_file);
    ...
    Client::diff()
{
    ...
    int fd = execute("diff " + m_log_file + ".ircserv " + m_log_file +
                     ".ergo > " + m_log_file + ".diff && rm " + m_log_file +
                     ".diff 2> /dev/null");
