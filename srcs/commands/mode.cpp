	...
	while (mode != mode_end && parameter_need_mode_count < 3)
	{
		switch (*mode)
		{
			...
			case 'p':
				...
				client.push_message(msg.rpl_channel_mode_is(channel_name, toggle, *mode), Logger::Debug);
				break;
			case 's':
			...
			case 'k':
				if (toggle == true)
				{
					...
					client.push_message(msg.rpl_channel_mode_is(channel_name, toggle, *mode, *parameter), Logger::Debug);
				}
				else
				{
