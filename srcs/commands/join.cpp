                           ...
                           ChannelKeyPairMap &chan_key_pair)
{
         ...
         ++channel_key_it) // 채널과 키쌍을 순회하면서 확인
    {
        ...
        if (!utils::is_channel_prefix(channel_name) ||
            !utils::is_channel_name_valid(
                channel_name)) // 채널이름 앞에 #, & 있는지, 이름이 유효한지
        {
        ...
        if (channel &&
            ((channel->is_key_mode()) &&
             (key != channel->get_key()))) // join할 채널 이름은 있는데 키가 안
                                           // 맞는 경우
        {
            client.push_message(msg.err_bad_channel_key(channel_name),
                                Logger::Debug);
            return;
        }
        else if (!channel) // join할 채널이 없는 경우(새로 만듦)
        {
            ...
            Logger().info()
                << "Create new channel :" << channel_name << " with " << key
                << " key by " << client.get_nickname();
        }
        else // join할 채널이 존재하는 경우
        {
            ...
            if ((channel->is_key_mode()) &&
                (key != channel->get_key())) // key mode인데 key가 안 맞을 때
            {
                client.push_message(msg.err_bad_channel_key(channel_name),
                                    Logger::Debug);
                return;
            }
            if (channel->is_limit_mode() &&
                channel->is_full()) // 현재 채널이 포함할 수 있는 최대 유저 수에
                                    // 도달했을 때
            {
            ...
            if (channel->is_invite_only_mode() &&
                !channel->is_user_on_invitation_list(
                    &client)) // invite-only인 경우
            {
                client.push_message(msg.err_invite_only_chan(channel_name),
                                    Logger::Debug);
                return;
            }
            channel->add_user(client);
            ...
            Logger().info() << "Join channel :" << channel_name << " with "
    ...
    Server::m_process_join_command(Client &client, Message &msg)
{
    ...
    if (msg.get_params().size() >= 2) // key가 있을 때
    {
        ...
        if ((channel_list.size() <
             key_list.size())) // key개수가 채널 개수보다 많을 때
        {
            client.push_message(msg.err_bad_channel_key(msg.get_command()),
                                Logger::Debug);
            return;
        }
