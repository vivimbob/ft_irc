    ...
    FT_IRC::m_pass(Message& msg)
{
    ...
    if (client.is_registered() && !_client_map.count(client.get_names().nick))
        _server->m_register_client(client, msg);
}

void
    FT_IRC::m_nick(Message& msg)
{
    Client& client = msg.get_from();
    ...
    {
        _server->m_send_to_channel(client, msg.build_nick_reply(nickname),
                                   &client);
    ...
    FT_IRC::m_quit(Message& msg)
{
    ...
    _server->m_disconnect_client(client, message);
}

void
    FT_IRC::m_join(Message& msg)
{
    Client& client = msg.get_from();
         ...
         channel_it != channel_list.end(); ++channel_it)
    {
        ...
        _server->m_send_to_channel(channel, msg.build_join_reply(channel_name));
...
void
    FT_IRC::m_mode_channel(Message& msg, const std::string& channel_name)
{
    Client& client = msg.get_from();
...
void
    FT_IRC::m_mode_user(Message& msg, const std::string& nickname)
{
    Client& client = msg.get_from();
    ...
    FT_IRC::m_invite(Message& msg)
{
    ...
    _server->m_prepare_to_send(*target_client,
                               msg.build_invite_reply(nickname, channel_name));
}

void
    FT_IRC::m_kick(Message& msg)
{
    Client& client = msg.get_from();
